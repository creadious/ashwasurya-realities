    ...
    return (
        <section className="md:py-20 py-10 md:px-20 px-4 text-center">
            ...
            <div className="md:mt-16 mt-10">
                <button onClick={() => setModal(true)} className="md:text-sm text-xs font-semibold hover:bg-black hover:text-white duration-200 px-4 py-2 md:border-2 border border-black">
                    DOWNLOAD BROCHURE
                </button>
            </div>
            <div className={`bg-white fixed h-screen md:w-1/2 w-full top-0 shadow-2xl duration-500 p-5 ${modal ? 'left-0' : '-left-[100rem]'}`} >
                <button onClick={() => setModal(false)} className="absolute left-5 top-5 hover:text-slate-600" title="close">
                    <ImCross />
                </button>
                <Brochure />
            </div>
